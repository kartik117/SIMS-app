...
db = Database('new_single_user1.db')
...
class ClassTreeviewII(tk.Frame):
    def __init__(self, parent): # I doubt this root arg is required
        ...
        self.my_tree.column("Class", anchor=tk.W, stretch=tk.NO, width=50)
    ...
    def populate_treeview(self):
        ...
        # Loop through records from database
         ...
         # Insert classes as child rows of courses
        for record in db.fetch_class():
            if self.count_class % 2 == 0:
                # Insert even color striped records into treeview
               self.my_tree.insert(parent="", index="end", iid=self.count_class, open=True, values=(record[0], record[1]), tags=("evenrow",)) # record[0] = class_id 'from database', record[1]= class_name
               ...
               self.my_tree.insert(parent="", index="end", iid=self.count_class, values=(record[0], record[1]), tags=("oddrow",))  # record[0] = class_id 'from database', record[1]= class_name
               # Insert students as child rows of each class.
               for rec in db.fetch_students_in_class(record[1]):
                   if self.count_student % 2 == 0:
                       self.my_tree.insert(parent=self.count_class, index="end", iid=self.count_student, values=('', student_index, rec[0], rec[1]), tags=("evenrow",))
   ...
   # Will also need a process to reverse enrollment                 
    ...
    for student in db.fetch_enrollment():
        if 
