            ...
            if self.count % 2 == 0:
               ...
               for enrol in db.fetch_enrollment(record[0]):
                   if self.count_class % 2 == 0:
                       self.my_tree.insert(parent=self.count, index="end", iid=self.count_class, values=(enrol[0], enrol[1], enrol[2], enrol[3], enrol[4]), tags=("oddrow",))
                        # Insert students as child rows of each class.
                       for rec in db.fetch_students_in_class(enrol[2]):
                       ...
                       self.my_tree.insert(parent=self.count, index="end", iid=self.count_class, values=(enrol[0], enrol[1], enrol[2], enrol[3], enrol[4]), tags=("evenrow",))
