...
class CourseTreeview(tk.Frame):
    def __init__(self, parent): # I doubt this root arg is required
        ...
        self.my_tree.column("ID", anchor=tk.CENTER, width=10)
        self.my_tree.column("Course name", anchor=tk.W, width=200)
        self.my_tree.column("Course code", anchor=tk.CENTER, width=600)
        ...
        add_course_frame = tk.LabelFrame(self, text="Add course")
        ...
        # Add Buttons
        add_button = ttk.Button(add_course_frame, text="Add Record", command=self.add_record, width=42)
        add_button.grid(row=0, column=4, padx=10, pady=5, columnspan=2)
        
        update_button = ttk.Button(add_course_frame, text="Update Record", width=19)
        update_button.grid(row=1, column=4, padx=1, pady=5)

        remove_one_button = ttk.Button(add_course_frame, text="Remove Record", command=self.remove_record, width=19)
    ...
    def populate_treeview(self):
        ...
        for record in db1.fetch_course():
    ...
    def add_record(self):
        ...
        if self.course_name_entry.get() == "":
            ...
            return
        db1.insert_course(self.course_name_entry.get(), self.course_code_entry.get())
        # Clear Treeview
    ...
    def update_record(self):
        ...
        db1.update_course(values[0], self.course_name_entry.get(), self.course_code_entry.get()) # values[0] = course_id
        # Clear entries
        self.clear_entries()
        # Clear Treeview
        for record in self.my_tree.get_children():
    ...
    def remove_record(self):
        ...
        db1.remove_course(values[1]) # values[1] = course_name
        # Clear entries
        self.clear_entries()
        # Clear Treeview
        for record in self.my_tree.get_children():
